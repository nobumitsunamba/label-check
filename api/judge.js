...
判定ルール
以下のいずれかに分類してください。
1. OK: ラベルが画像内に明確に1枚確認でき、正規の認定ラベル（青楕円・低排出ガス車の文字）であり、かつ「国土交通大臣認定車」と記載されている
2. 要確認: 以下のいずれかに該当する場合
   * ラベルらしきものは見えるが、不鮮明・ぼけ・反射などで確信できない
   * ラベルが複数枚写っている
3. NG: 以下のいずれかに該当する場合
   * ラベルが画像内に存在しない、または全く別のものが写っている
   * 「国土交通大臣“非”認定車」と記載されている（＝大臣非認定のため正規の認定ラベルではない）
   * 「認定車」ではない、認定を示さない偽物・パロディ等のラベルである

認定/非認定の判定（最重要）
* ラベル下部の「国土交通大臣○認定車」の文字を必ず読み取ってください。
* 「認定車」なら認定、「非認定車」なら非認定です。「非」の一文字を絶対に見落とさないでください。
* 非認定車と記載がある場合は、星の数や見た目が正規ラベルに似ていても必ず NG としてください。
...
年度・低減率の読み取り
* 「平成○年」または「令和○年」の表記を読み取り、year に文字列で入れてください（例: "平成20年"）。読み取れない場合は null。
* 「排出ガス基準○％低減」の○％を読み取り、reduction に文字列で入れてください（例: "50%"）。読み取れない場合は null。

レスポンス形式
必ず以下のJSON形式のみで返答してください。他のテキストは含めないでください。
{ "judgment": "OK" | "要確認" | "NG", "stars": 0〜5の整数（上記「星の数え方」に従って正確に数えた数。判定がNGまたは要確認でラベルが不明な場合はnull）, "year": "平成○年"または"令和○年"の文字列（読み取れない場合はnull）, "reduction": "○%"の文字列（読み取れない場合はnull）, "confidence": 0〜100の整数（判定の確信度）, "reason": "判定根拠を1〜3文で簡潔に。認定/非認定の別と、星を左から数えた結果を必ず含めること" }`;
